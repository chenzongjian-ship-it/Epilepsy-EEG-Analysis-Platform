...
本平台用于展示癫痫脑电信号自动分析的核心流程。用户批量上传 EDF 文件后，页面会在浏览器端读取 EDF header 和通道采样，并执行以下链路：

1. 数据读取：解析 EDF header、通道信息、采样率、数字量程和物理量程。
2. 预处理：对真实采样序列进行均值去除、标准化和平滑滤波。
3. 特征提取：计算 RMS、线长、θ/α/β 频带能量、θ/β、谱熵和 Hjorth 指标。
4. 模型训练：使用前端轻量规则模型根据特征估计发作概率。
5. 结果评估：输出每个 EDF 文件的概率、标签和批量汇总指标。
6. 报告导出：生成包含文件级特征和预测结果的 JSON 分析摘要。
