...
这是一个用于课程项目展示的癫痫脑电分析平台前端原型。平台围绕 CHB-MIT 等 EDF 脑电数据的分析流程，展示从数据读取到报告导出的完整结果链，适合放入 GitHub 仓库作为项目主页和作品展示入口。
...
本平台用于展示癫痫脑电信号自动分析的核心流程。用户上传 EDF 文件后，页面会模拟执行以下链路：
...
| 模块 | 说明 |
| --- | --- |
| 数据导入 | 上传 EDF 脑电文件，触发分析流程 |
| 预处理 | 展示原始/滤波 EEG 波形 |
| 特征提取 | 展示 RMS、θ Power、SampEn、HFD 等特征 |
| 模型训练 | 展示发作识别模型输出 |
...
平台界面包含左侧功能导航区、中间 EDF 上传区、流程进度条、波形图、时频图、特征表、模型输出和识别报告区。
